...
module.exports = function Users () {
	...
	function addUser (socket, name, type) {
		...
		events.userAccepted(socket, name, userNames());
	...
	function addPrevious (newSocket, previousID) {
		...
		events.userAccepted(newSocket, previous.name, userNames());

	}

	// Sets up the client as one of the three types of user.
	function userType (socket, type, previousID) {
		...
		if (type === 'player') {
			...
			if (previousID in previousPlayers) {
				addPrevious(socket, previousID);
			} else {
				addUser(socket, `Player ${metadata.noPlayers}`, type);
			}
